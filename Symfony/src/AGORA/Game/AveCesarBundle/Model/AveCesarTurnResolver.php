...
use AGORA\Game\AveCesarBundle\Model;
...
class AveCesarTurnResolver {
    ...
    public function move($gameId, $playerId, $position, $card) {
        ...
        $occupiedPositions = array_map(function ($p) {return $p->getPosition();}, $otherPlayers);

        //$hand = preg_split("/,/", $player->getHand());
        $this->service->flush();
        $hand = $this->service->getHand($playerId);

        if ($this->service->isFirstPlayer($playerId, $gameId)
                && $card == 6
                && !$this->firstCanPlaySix($hand, $player->getPosition())) {
        ...
        if ($this->canMoveTo($player->getPosition(), $position, $occupiedPositions, $card)) {
    ...
    private function canPass($playerId, $gameId, $startPosition, $occupiedPositions, $cards) {
        ...
        if ($firstId == $playerId && $this->isHandOf($hand, "6") && !$this->isStuckPosition($startPosition)) {
            return true;
        }
        $result = true;
        if ($firstId == $playerId) {
            foreach ($cards as $c) {
                if ($c != "6") {
                    $result = $result && !$this->canMove($startPosition, $occupiedPositions, $c);
                }
            }
        } else {
            foreach ($cards as $c) {
    ...
    private function canMove($position, $occupiedPositions, $moveCount) {
        echo $position."\n";
        ...
        $result = false;
        foreach ($this->boardMap["$position"] as $nextPosition) {
            $result = $result || $this->canMove($nextPosition, $occupiedPositions, $moveCount - 1);
        }
        return $result;
    }

    private function canMoveTo($position, $targetPosition, $occupiedPositions, $moveCount) {
        echo $position."\n";
        if ($this->isOccupied($position, $occupiedPositions)) {
            return false;
        }
        ...
        $result = false;
        foreach ($this->boardMap["$position"] as $nextPosition) {
            $result = $result || $this->canMoveTo($nextPosition, $targetPosition, $occupiedPositions, $moveCount - 1);
        }
        return $result;
    }
    ...
    private function updateFinish($player, $game) {
        if ($player->getLap() >= 4) {
            ...
            $ranks = array_map(function ($p) {return $p->getFinish();}, $players);
            $max = array_reduce($ranks, function($a, $b) {return max($a, $b);}, 0);
            $player->setFinish($max + 1);
            $nbFinish = array_reduce($ranks,
                function ($carry, $rank) {
                    if ($rank > 0) {
                        return $carry + 1;
                    }
            ...
            if ($nbFinish + 1 >= count($players) - 1) {
                $this->service->setGameState($game->getId(),"finish");
                //$this->service->finishGame($game->getId());
            }
        }
    }
     ...
     * @return bool
     */
    private function isStuckPosition($position) {
        $stuckPosition = array("7a", "17a", "25a");
        return in_array($position, $stuckPosition);
    }
     ...
     */
    private function firstCanPlaySix($hand, $position) {
        if ($this->isStuckPosition($position)) {
